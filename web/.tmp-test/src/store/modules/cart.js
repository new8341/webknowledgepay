...
import { apiCheckoutSelected, apiDeleteCartSelected, apiDeleteCartItem, apiFetchCart, apiSetCartItemQuantity, apiSetCartItemSelected, } from '../../api';
export default {
    namespaced: true,
    state: () => ({
        items: [
    ...
    mutations: {
        ...
        setPaymentMethod(localState, method) {
            localState.paymentMethod = method;
        },
        setCheckoutStep(localState, step) {
            localState.checkoutStep = step;
        },
        setCouponType(localState, couponType) {
            localState.couponType = couponType;
        },
        setCouponCode(localState, couponCode) {
    ...
    actions: {
        ...
        async setItemSelected({ commit }, payload) {
            commit('setLoading', true);
            commit('setError', null);
            try {
                const res = await apiSetCartItemSelected(payload.itemId, payload.selected);
                commit('setItems', res.data.items);
            }
            catch (err) {
                const normalized = toApiErrorPayload(err, {
        ...
        async toggleSelectedAll({ state, dispatch, commit }, selected) {
            commit('setError', null);
        ...
        async setItemQuantity({ commit }, payload) {
            commit('setLoading', true);
            commit('setError', null);
            try {
                const res = await apiSetCartItemQuantity(payload.itemId, payload.quantity);
                commit('setItems', res.data.items);
            }
            catch (err) {
                const normalized = toApiErrorPayload(err, {
        ...
        async deleteSelected({ commit }) {
            ...
            commit('setError', null);
            try {
                const res = await apiDeleteCartSelected();
                commit('setItems', res.data.items);
            }
            catch (err) {
                const normalized = toApiErrorPayload(err, {
        ...
        setCouponType({ commit }, couponType) {
        ...
        async checkout({ state, commit, dispatch }) {
            commit('setError', null);
            const selectedCount = state.cart.items.filter((it) => it.selected).length;
            if (selectedCount === 0) {
